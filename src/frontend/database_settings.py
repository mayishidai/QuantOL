...
def show_database_settings_page():
    """显示数据库设置页面"""
    st.title("🔧 数据库设置")
    ...
    # 显示当前数据库状态，并自动初始化SQLite如果需要
    show_current_database_status(auto_init_sqlite=True)
...
def show_current_database_status(auto_init_sqlite=False):
    """显示当前数据库状态"""
    current_type = get_database_type()
                    ...
                    try:
                        ...
                        if loop.is_running():
                            ...
                            with concurrent.futures.ThreadPoolExecutor() as executor:
                                future = executor.submit(asyncio.run, _init_sqlite_database())
                                future.result()
                        else:
                            # 如果循环没有运行，直接使用
                            loop.run_until_complete(_init_sqlite_database())
                    except RuntimeError:
                        # 如果没有事件循环，创建新的
                ...
                try:
                    ...
                    try:
                        ...
                        if loop.is_running():
                            ...
                            import concurrent.futures
                            with concurrent.futures.ThreadPoolExecutor() as executor:
                                future = executor.submit(asyncio.run, reinit_sqlite_database())
                                future.result()
                        else:
                            # 如果循环没有运行，直接使用
                            loop.run_until_complete(reinit_sqlite_database())
                    except RuntimeError:
                        # 如果没有事件循环，创建新的
                ...
                try:
                    ...
                    try:
                        ...
                        if loop.is_running():
                            ...
                            import concurrent.futures
                            with concurrent.futures.ThreadPoolExecutor() as executor:
                                future = executor.submit(asyncio.run, test_postgresql_connection())
                                result = future.result()
                        else:
                            # 如果循环没有运行，直接使用
                            result = loop.run_until_complete(test_postgresql_connection())
                    except RuntimeError:
                        # 如果没有事件循环，创建新的
